...
def init_db(con):
    ...
    cursorObj.execute(f"CREATE TABLE TIMETRACK(user text, StartTime date, FinishTime date)")
    con.commit()
    con.close()

def record_start_time(con, user):
    cursorObj = con.cursor()
    cursorObj.execute(f"INSERT INTO TIMETRACK(user, StartTime) VALUES('{user}', datetime('now'))")
    con.commit()

def record_finish_time(con, user):
    cursorObj = con.cursor()
    cursorObj.execute(f"UPDATE TIMETRACK SET FinishTime=datetime('now') WHERE rowid=(SELECT rowid FROM TIMETRACK WHERE user = '{user}' AND FinishTime IS NULL ORDER BY StartTime DESC LIMIT 1 )")
    con.commit()
...
def message_okay(message, say):
    say(f"Хорошо, <@{message['user']}>!")
