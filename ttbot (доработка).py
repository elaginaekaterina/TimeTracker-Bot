...
def record_finish_time_user(con, user, time):
    exec_query(con, f"UPDATE TIMETRACK SET FinishTime=datetime(date('now','localtime')||' {time}') WHERE rowid=(SELECT rowid FROM TIMETRACK WHERE user = '{user}' AND FinishTime IS NULL ORDER BY StartTime DESC LIMIT 1)")
...
def get_stats(ack, con, startPeriod, endPeriod):
    ...
    for row in rows:
        ...
        stats += f"{result['user']['real_name']} \t {row[1]}\n"
    ack(stats)
